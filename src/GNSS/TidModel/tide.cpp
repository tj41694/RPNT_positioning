...
#define GMS         1.327125E+20    /* sun gravitational constant */
#define GMM         4.902789E+12    /* moon gravitational constant */

/* earth, ocean loading and pole tide function ---------------------------------------------------- */
/* Constructors ------------------------------------------------------------------- */
gnss_tidecorr_c::gnss_tidecorr_c() {
    tut=gtime_c();
    for (int i=0; i<3; i++) {
        C_e_n[i*3]=C_e_n[i*3+1]=C_e_n[i*3+2]=0.0;
...
void gnss_tidecorr_c::iers_mean_pole(double *xp_bar,double *yp_bar) {
    const double ep2000[]={ 2000,1,1,0,0,0 };
    double y,y2,y3;

    gtime_c t2000(ep2000);
...
void gnss_tidecorr_c::tide_sl(const double *eu,const double *rp,double GMp,
...
void gnss_tidecorr_c::tide_pole() {
...
void gnss_tidecorr_c::tide_ocean(int rovbas) {
...
void gnss_tidecorr_c::tide_solid() {
    double dr1[3],dr2[3],eu[3],du,dn,sinl,sin2l;

    /* step1: time domain */
    eu[0]=C_e_n[6]; eu[1]=C_e_n[7]; eu[2]=C_e_n[8];
    ...
    mode_dr[0][0]=dr1[0]+dr2[0]+du*C_e_n[6];
    mode_dr[0][1]=dr1[1]+dr2[1]+du*C_e_n[7];
    mode_dr[0][2]=dr1[2]+dr2[2]+du*C_e_n[8];
    ...
    if (tide_opt&8) {
        ...
        dn=0.0247*sin2l;
        mode_dr[0][0]+=du*C_e_n[6]+dn*C_e_n[3];
        mode_dr[0][1]+=du*C_e_n[7]+dn*C_e_n[4];
        mode_dr[0][2]+=du*C_e_n[8]+dn*C_e_n[5];
    }
}
/* public funtions ---------------------------------------------------------------- */
/* initialize ocean loading parameter pointer ------------------------------------- */
void gnss_tidecorr_c::init_otl(gnss_prcopt_c *opt,gnss_nav_c *nav) {
    tide_opt=opt->tidecorr;
    ocean_par[0]=nav->ocean_par[0];
    ocean_par[1]=nav->ocean_par[1];
}
/* initialize earth rotation parameter pointor ------------------------------------ */
void gnss_tidecorr_c::init_erp(gnss_nav_c *nav) {
    er_par=&nav->erp;
}
/* compute tidal displacement corretion ----------------------------------------------
* args   : gtime_c time     I   time in utc
...
* --------------------------------------------------------------------------------- */
void gnss_tidecorr_c::tidecorr(gtime_c time,const int rovbas,const double *xyz,double *dr) {
    ...
    if ((tide_opt&2)&&ocean_par[rovbas]) {
        tide_ocean(rovbas);
        matmul_pnt("TN",3,1,3,1.0,C_e_n,denu[0],0.0,mode_dr[1]);
        for (int i=0; i<3; i++) dr[i]+=mode_dr[1][i];
    }
    /* pole tide */
    if (tide_opt&4) {
        tide_pole();
        matmul_pnt("TN",3,1,3,1.0,C_e_n,denu[1],0.0,mode_dr[2]);
