    ...
    for (i = n = 0; (i < nsat) && (i < MAXOBS); i++) {
        ...
        if (rif & M_CONCISE) {
            ...
            p += 2; /* I2 Satellite Azimuth (degrees) */
                    ...
                    /* L1 data valid */
            if (Flags1 & M_BIT6) {
                /* Measure of L1 signal strength (dB) */
                obs.data[n].SNR[0] = U1(p)*0.25;
                p++;
        ...
        if (ClockOffset != 0.0) {
            obs.value[n].P[0] += ClockOffset * (CLIGHT/FREQ1);
            obs.value[n].P[1] += ClockOffset * (CLIGHT/FREQ2);
            obs.value[n].L[0] += ClockOffset * FREQ1;
            obs.value[n].L[1] += ClockOffset * FREQ2;
        }
#endif
        n++;
    }
