...
* --------------------------------------------------------------------------------- */
double gnss_ioncorr_c::ionppp(const double pos[3],const gnss_obsd_c *obs,const double re,
...
double gnss_ioncorr_c::ionmapf(const double *pos,const double *azel) {
...
* --------------------------------------------------------------------------------- */
int gnss_ioncorr_c::correction(gnss_obsd_c *obs,const gnss_nav_c *nav,const double pos[3]) {
    obs->ionmap=1.0;
    obs->dion=0.0;
...
gnss_LCion_c::gnss_LCion_c() {
}
gnss_LCion_c::~gnss_LCion_c() {
}
/* return 0 delya ----------------------------------------------------------------- */
int gnss_LCion_c::correction(gnss_obsd_c *obs,const gnss_nav_c *nav,const double pos[3]) {
    obs->ionmap=0.0;
    obs->dion=0.0;
...
/* Constructors ------------------------------------------------------------------- */
gnss_broadion_c::gnss_broadion_c() {
}
gnss_broadion_c::~gnss_broadion_c() {
}
/* Implementation functions ------------------------------------------------------- */
...
* pos[3]   : lat lon h {rad,m}
* --------------------------------------------------------------------------------- */
int gnss_broadion_c::klobion(gnss_obsd_c *obs,const double ionpara[8],const double pos[3]) {
    double tt,psi,phi,lam,amp,per,x;
    int week;
    gtime_c obstime=obs->time;
...
* --------------------------------------------------------------------------------- */
int gnss_broadion_c::correction(gnss_obsd_c *obs,const gnss_nav_c *nav,const double pos[3]) {
    if (!klobion(obs,nav->ion_gps,pos)) SQR(obs->dion*ERR_BRDCI);
...
* pos[3]   : lat lon h {rad,m}
* --------------------------------------------------------------------------------- */
int gnss_qzssion_c::correction(gnss_obsd_c *obs,const gnss_nav_c *nav,const double pos[3]) {
    if (!klobion(obs,nav->ion_qzs,pos)) SQR(obs->dion*ERR_BRDCI);
    obs->ionvar=SQR(obs->dion*ERR_BRDCI); /* ERR_BRDCI can be redefined */
    return 1;
}
...
/* base functions ----------------------------------------------------------------- */
/* search igps -------------------------------------------------------------------- */
void gnss_sbasion_c::searchigp(gtime_c time,const double pos[2],const gnss_sbsion_c *ion,
    const gnss_sbsigp_c **igp,double &x,double &y) {
    int i,latp[2],lonp[4];
    double lat=pos[0]*R2D,lon=pos[1]*R2D;
    const gnss_sbsigp_c *p;
...
double gnss_sbasion_c::varicorr(int udre) {
    const double var[14]={
...
* pos[3]   : lat lon h {rad,m}
* --------------------------------------------------------------------------------- */
int gnss_sbasion_c::correction(gnss_obsd_c *obs,const gnss_nav_c *nav,const double pos[3]) {
    const double re=6378.1363,hion=350.0;
    int i,err=0;
    double x=0.0,y=0.0,t,w[4]={ 0 };
    const gnss_sbsigp_c *igp[4]={ 0 }; /* {ws,wn,es,en} */

    obs->dion=obs->ionvar=0.0;
    if (pos[2]<-1E3||obs->azel[1]<=0) return 1;

    /* ipp (ionospheric pierce point) position */
    obs->ionmap=ionppp(pos,obs,re,hion,obs->ipp);

    /* search igps around ipp */
    searchigp(obs->time,obs->ipp,nav->sbsion,igp,x,y);
    ...
    if (err) {
        obs->errorMsg="no sbas iono correction!";
        return 0;
    }
    for (i=0; i<4; i++) {
        if (!igp[i]) continue;
...
/* Constructors ------------------------------------------------------------------- */
gnss_ionexion_c::gnss_ionexion_c() {
}
gnss_ionexion_c::~gnss_ionexion_c() {
}
/* Implementation functions ------------------------------------------------------- */
...
int gnss_ionexion_c::dataindex(int i,int j,int k,const int *ndata) {
    if (i<0||ndata[0]<=i||j<0||ndata[1]<=j||k<0||ndata[2]<=k) return -1;
    return i+ndata[0]*(j+ndata[1]*k);
}
/* interpolate tec grid data ---------------------------------------------- */
int gnss_ionexion_c::interptec(const gnss_tec_c *tec,int k,const double *posp,double &value,
    double &rms) {
...
int gnss_ionexion_c::iondelay(const gnss_tec_c *tec,const double *pos,gnss_obsd_c *obs,
    double *delay,double *var) {
    const double fact=40.30E16/FREQ1/FREQ1; /* tecu->L1 iono (m) */
    double vtec,rms,hion;
    int i;
...
* pos[3]   : lat lon h {rad,m}
* --------------------------------------------------------------------------------- */
int gnss_ionexion_c::correction(gnss_obsd_c *obs,const gnss_nav_c *nav,const double pos[3]) {
    ...
    if (i==0||i>=nav->nt) {
        obs->errorMsg="tec grid out of period!";
        return 0;
    }
    if ((tt=nav->tec[i].time.timediff(nav->tec[i-1].time))==0.0) {
        obs->errorMsg="tec grid time interval error!";
        return 0;
    }
    ...
    if (!stat[0]&&!stat[1]) {
        obs->errorMsg="tec grid out of area pos!";
        return 0;
    }
...
/* Constructors ------------------------------------------------------------------- */
gnss_lexioncor_c::gnss_lexioncor_c() {
}
gnss_lexioncor_c::~gnss_lexioncor_c() {
}
/* Implementation functions ------------------------------------------------------- */
...
* pos[3]   : lat lon h {rad,m}
* --------------------------------------------------------------------------------- */
int gnss_lexioncor_c::correction(gnss_obsd_c *obs,const gnss_nav_c *nav,const double pos[3]) {
    ...
    if (fabs(tt)>nav->lexion.tspan) {
        obs->errorMsg="lex iono age error!";
        return 0;
    }
    ...
    lonpp=pos[1]+atan2(sinap*sinaz,(cosap*coslat-sinap*cosaz*sinlat));
...
/* Constructors ------------------------------------------------------------------- */
gnss_estion_c::gnss_estion_c() {
}
gnss_estion_c::~gnss_estion_c() {
}
/* compute constrained ionosphere model correction (vertical delay of GPS L1)------ */
int gnss_estion_c::correction(gnss_obsd_c *obs,const gnss_nav_c *nav,const double pos[3]) {
    int flag=0;
    /* return constrained ionex model corrections */
    if (nav->nt>2) flag=gnss_ionexion_c::correction(obs,nav,pos);

    /* return constrained broadcast model corrections */
    else if (norm(nav->ion_gps,8)>=0) flag=gnss_broadion_c::correction(obs,nav,pos);
