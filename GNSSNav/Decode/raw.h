...
/* Constant */
/* half-cycle correction list structure ----------------------------------------------------------- */
typedef struct {
    ...
    gtime_c ts,te;                      /* time start, time end */
} half_cyc_t;
...
class raw_c : public decode_data_c {
/* Constructor */
public:
    raw_c();
    virtual ~raw_c();
...
protected:
    ...
    int decode_gal_inav(const int sat,gnss_eph_c &eph);
    /* decode BeiDou D1 ephemeris --------------------------------------------- */
    int decode_bds_d1(const int sat,gnss_eph_c &eph);
    /* decode BeiDou D2 ephemeris --------------------------------------------- */
    int decode_bds_d2(const int sat,gnss_eph_c &eph);
    /* test hamming code of glonass ephemeris string -------------------------- */
    int test_glostr(const unsigned char *bbuff);
    /* decode glonass ephemeris strings --------------------------------------- */
    int decode_glostr(const int sat,gnss_geph_c &geph);
...
/* Components */
public:
    gtime_c tobs;                       /* observation data time */
    gnss_obs_c obuf;                    /* observation data buffer */
