...
/* convert signal to frequency and obs class ------------------------------------------------------ */
...
gtime_c javad::adjweek(gtime_c ttt,double tow) {
    double tow_p;
    int week;
    gtime_c adjt;
...
gtime_c javad::adjday(gtime_c ttt,double ttod) {
    double tod_p;
    gtime_c adjt;
...
int javad::decode_xP(char code) {
    ...
    for (i=0; i<obuf.n&&i<MAXOBS; i++) {
        ...
        if ((j=checkpri(sys,type,freq))>=0) {
            ...
            fn=sys==SYS_GLO ? freq_glo(freq,freqn[i]) : GNSS_FREQ_Hz[iGPS][ GNSS_USED_FREQ_DEF[iGPS][freq] ];
...
int javad::decode_xd(char code) {
    ...
    for (i=0; i<obuf.n&&i<MAXOBS; i++) {
        ...
        if ((j=checkpri(sys,type,freq))>=0) {
            if (!settag(i)) continue;
            f1=sys==SYS_GLO ? freq_glo(0,freqn[i]) : GNSS_FREQ_Hz[iGPS][1];
            fn=sys==SYS_GLO ? freq_glo(freq,freqn[i]) : GNSS_FREQ_Hz[iGPS][ GNSS_USED_FREQ_DEF[iGPS][freq] ];
...
int javad::decode_javad() {
    ...
    if (!strncmp(p,"cd",2)) return decode_nd(SYS_BDS); /* raw navigation data */
