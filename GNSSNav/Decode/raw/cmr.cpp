...
| and rover observation ins_timesys. It must be set low enough so that the change in
...
/* Utility macros: */
#define SNRATIO(snr) ((snr<=0.0)||(snr>=64))?(0.0):(snr)
...
static gtime_c CmrTimeToGtime(unsigned int CmrTime);
static gtime_c DoubleToGtime(double Time);
static double GtimeToDouble(gtime_c Time);
static const char *ReceiverNumberToName(unsigned short Number);
static gtime_c ReferenceCmrTime(gtime_c CmrTime,gtime_c RoverTime,double WindowSize);
...
/* Convert a double to a gtime_c time ------------------------------------------------------------- */
static gtime_c DoubleToGtime(double Double) {
    gtime_c Gtime;
    Gtime.time = floor(Double);
    Gtime.sec = Double - Gtime.time;
    return Gtime;
}
/* Convert CMR time in milliseconds to a gtime_c time --------------------------------------------- */
static gtime_c CmrTimeToGtime(unsigned int CmrTime) {
    return DoubleToGtime(CmrTime*0.001);
}
/* Convert a gtime_c time to a double ------------------------------------------------------------- */
static double GtimeToDouble(gtime_c Gtime) {
...
int cmr::ReferenceCmrObs(gtime_c Time,unsigned char Type,double P0,obsbd_t *b) {
    obsbd_t *t4 = T4Data;
    gnss_nav_c *Nav = (Svr) ? Svr->nav : &nav;
    ...
    if (Type == CMR_TYPE_0) {
        ...
        if ((L1WaveLength = Nav->satwavelen(b->Sat,0) == 0.0) ||
            (L2WaveLength = Nav->satwavelen(b->Sat,1) == 0.0))
        {
            return -1;
        }
    }
...
int cmr::DecodeCmrType0() {
    ...
    gtime_c CmrTime = CmrTimeToGtime(ubitn(p+4,6,18));
...
int cmr::DecodeCmrType4() {
    obsbd_t *t4 = T4Data;
    unsigned char *p = (unsigned char*)&MessageBuffer[4];
    gtime_c CmrTime = CmrTimeToGtime(ubitn(p+4,6,10)<<2);
...
int cmr::update_cmr() {
    ...
    for (n = 0; (n < obs.n) && (n < MAXOBS); n++) {
        ...
        if (Sat<MAXSAT && obs.data[n].time.timediff(RoverObservables[Sat].Time)>0) {
