...
gtime_c binex::adjweek(gtime_c ttt,double tow) {
    double tow_p;
    int week;
    gtime_c adjw;
...
gtime_c binex::adjday(double ttod) {
    double tod_p;
    gtime_c adjd;
...
unsigned char * binex::decode_bnx_7f_05_obs(unsigned char *bbb,
    int sat,int nobs,gnss_obsd_c &data) {
    ...
    const unsigned char codes_cmp[32]={
        CODE_L2X ,CODE_L2I ,CODE_L2Q ,CODE_L1X ,CODE_L7X ,CODE_L7I , /*  0- 5 */
        CODE_L7Q ,CODE_L7X ,CODE_L6X ,CODE_L6I ,CODE_L6Q ,CODE_L6X , /*  6-11 */
        CODE_L1D ,CODE_L1S ,CODE_L1L ,CODE_L1P                       /* 12-15 */
    };
    ...
    switch (sys) {
        ...
        case SYS_BDS: codes=codes_cmp; break;
    }
    for (i=0; i<nobs; i++) {
    ...
    /* get code priority */
    for (i=0; i<nobs; i++) {
        code2obs(codes[code[i]&0xFF],freq+i);
        pri[i]=getcodepri(sys,codes[code[i]&0xFF],opt);
